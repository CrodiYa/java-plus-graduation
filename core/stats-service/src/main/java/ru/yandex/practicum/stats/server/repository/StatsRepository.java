...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    @Query("""
            SELECT new ru.yandex.practicum.interaction.stats.dto.ViewStatsDto(hit.app, hit.uri, COUNT(DISTINCT hit.ip))
    ...
    @Query("""
            SELECT new ru.yandex.practicum.interaction.stats.dto.ViewStatsDto(hit.app, hit.uri, COUNT(hit))
            FROM EndpointHit as hit
            WHERE hit.createDate BETWEEN :start AND :end AND hit.uri IN :uris
            GROUP BY hit.app, hit.uri
